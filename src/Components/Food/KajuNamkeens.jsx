...
import KajuNamkeen from "../ShopsData/KajuNamkeenData";
...
function KajuNamkeens({ filter, sortBy, price, cuisine }) {
    const navigate = useNavigate();
    const swiperRef = useRef(null);

    const { cartItems, addToCart, updateQty, getCartKey } = useCart();

    const list = useMemo(() => KajuNamkeen, []);
    ...
    const placeOrder = (item) => {
        navigate("/order", {
            state: {
                ...
                category: "kajunamkeen",
            },
        });
    };

    return (
        <div id="kajunamkeen" className="food-container">
            <div className="food-text">
                <h1>Kaju Nakmeen</h1>
                <p
                    className="view-all"
                    onClick={() => navigate("/all-kaju-namkeen")}
