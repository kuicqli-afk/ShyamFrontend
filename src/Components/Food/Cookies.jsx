...
import cookiesList from "../../data/CookiesData";
...
const Cookies = ({ filter, sortBy, price, cuisine }) => {
  ...
  const list = useMemo(() => cookiesList, []);
  ...
  const getQty = (item) => {
    ...
    return found ? found.quantity : ("ADD"); // Default to 1 if not found
  };
  ...
  const placeOrder = (item) =>
    navigate("/order", {
      state: {
        ...
        category: "cookies", // Set category to "cookies"
      },
    });

  /* ✅ LOOP STABLE: ensure enough slides for infinite loop */
  // const list = useMemo(() => {
  //   const base = cookiesList || [];
  //   if (base.length === 0) return [];
  //   let result = [...base];
  //   while (result.length < 15) result = [...result, ...base];
  //   return result;
  // }, []);

  return (
    <div id="cookies" className="food-container">
      <div className="food-text">
        <h1>Cookies</h1>
        <p className="view-all" onClick={() => navigate("/all-cookies")}>View all</p>
      </div>
      {/* <div>
