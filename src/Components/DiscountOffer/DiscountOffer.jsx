...
const DiscountOffer = () => {
  ...
  const placeOrder = (item) =>
    navigate("/order#", { state: { item, category: "offers" } });
  ...
  const tag = query.get("tag"); // offer / combo / weekend
  const discount = query.get("discount"); // only for offer
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        // ✅ OFFER
        // ✅ OFFER
        if (tag === "offer" && discount) {
          url += `?tag=offer&discount=${discount}`;
        }
  ...
  const getTitle = () => {
    if (tag === "offer") return "Discount Offers";
    if (tag === "combo") return "Combo Deals";
    if (tag === "weekend") return "Weekend Specials";
    return "All Products";
  };
  ...
  return (
    ...
    <div className="food-container">
      <div id="offers" className="food-text">
        <h1>{getTitle()}</h1>
      </div>
