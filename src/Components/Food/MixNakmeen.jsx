...
function MixNakmeen({ filter, sortBy, price, cuisine }) {
    const navigate = useNavigate();
    const swiperRef = useRef(null);

    const { cartItems, addToCart, updateQty, getCartKey } = useCart();

    const list = useMemo(() => ShyamList, []);
    ...
    return (
        <div id="namkeen" className="food-container">
            <div className="food-text">
                <h1>Mix Namkeen</h1>
                <p className="view-all" onClick={() => navigate("/all-mix-namkeen")}>View all</p>
            </div>
            {/* div>
