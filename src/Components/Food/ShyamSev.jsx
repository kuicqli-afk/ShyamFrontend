...
import SevList from "../../Components/ShopsData/ShyamSevData"; // only biryani items
// import "./Food.css";
...
const ShyamSev = ({ filter, sortBy, price, cuisine }) => {
  const navigate = useNavigate();
  const swiperRef = useRef(null);

  const { cartItems, addToCart, updateQty, getCartKey } = useCart();

  const list = useMemo(() => SevList, []);
  ...
  const placeOrder = (item) =>
    navigate("/order", {
      state: {
        ...
        category: "sev", // Category set to "rolls"
      },
    });
  return (
    <div id="sev" className="food-container">
      <div className="food-text">
        <h1>Sev</h1>
        <p className="view-all" onClick={() => navigate("/all-sev")}>View all</p>
      </div>
      {/* <div>
