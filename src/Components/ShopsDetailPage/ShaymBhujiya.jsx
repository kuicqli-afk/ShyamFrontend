...
// import ShyamBhujiyaData from "../../Components/ShopsData/ShyamBhujiyaData"; 

function ShaymBhujiya() {
    ...
    useEffect(() => {
        const fetchBhujiya = async () => {
            try {
                const res = await axios.get(
                    `http://localhost:5000/api/products/category/${CATEGORY_ID}`
                );
    ...
    const placeOrder = (item) =>
        navigate("/order#", { state: { item, category: "bhujiya" } });
    return (
        <div className="food-container">
            <div id="bhujiya" className="food-text">
                <h1>Bhujiya</h1>
            </div>
                                            ...
                                            {Array.isArray(item.slots) &&
                                                item.slots.map((slot) => (
                                                    <li
                                                        key={slot}
                                                        className={
                                                            selectedSlots[item._id] === slot ? "selected" : ""
                                                        }
                                                        onClick={() =>
                                                            setSelectedSlots({
                                                                ...selectedSlots,
